  ...
  const addVocabWord = async () => {
    ...
    if (!currentVocabWordsList) {
      ...
      await AsyncStorage.setItem(
        "vocabWords",
        JSON.stringify(newVocabWordsList)
      )
        .then(() => {
        ...
        .catch((e) => {
          console.log(
            "There was an error while creating the vocab words list: ",
            e
          );
        });
    } else {
      ...
      await AsyncStorage.setItem(
        ...
        .catch((e) => {
          console.log(
            "There was an error while setting the vocab words list: ",
            e
          );
        });
    }
    ...
    return props.translatedWordList.map((data, index) => {
      return (
        <View key={index}>
          ...
          <Text style={styles.definitionsList}>
            {props.definitionsList[index]}
          </Text>
        </View>
      );
    });
